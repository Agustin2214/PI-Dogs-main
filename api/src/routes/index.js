...
const getApi = async () => {
  const apiUrl = await axios.get(
    `https://api.thedogapi.com/v1/breeds?${APY_KEY}`
  );
...
router.get("/dog", async (req, res) => {
  ...
  if (temperamentoss) {
          ...
          .json([
            {
              name: "NO ENCONTRADO",
              imagen:
                "https://media.istockphoto.com/photos/pug-dog-with-yellow-constructor-safety-helmet-and-cone-and-404-error-picture-id687810238?k=20&m=687810238&s=612x612&w=0&h=5MdWpmZyAKqnZxbUQw5d2t5KttUL4947Sn-DFdM8ayE=",
    ...
    let dogName = await razasTotales;
    dogName.length > 0
  ...
  if (razaid) {
    ...
    dogName
      ...
      : res
          .status(404)
          .send({
            name: "Lo siento, no pudimos encontrar esta tipo de raza, puedes crearla en Create Dog",
            imagen:
              "https://media.istockphoto.com/photos/pug-dog-with-yellow-constructor-safety-helmet-and-cone-and-404-error-picture-id687810238?k=20&m=687810238&s=612x612&w=0&h=5MdWpmZyAKqnZxbUQw5d2t5KttUL4947Sn-DFdM8ayE=",
...
router.post("/dog", async (req, res) => {
  let { name, height, weight, life_span, imagen, bred_for, temperament } =
    req.body;
  console.log(name);
...
let razasTotales = await getAllDog();
  ...
  const aux = z();
  const aux1 = aux.filter((e) =>e.name.toLowerCase().includes(name.toLowerCase()))

  aux1.length > 0
    ? res.status(200).send(aux1)
    : res.status(404)
        .json([
          {
            name: "NO ENCONTRADO",
      ...
      let dogName = await razasTotales.filter((e) =>
